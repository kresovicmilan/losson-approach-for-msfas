...
NEW_DATASET_DIR = GENERAL_DATASET_DIR + '\\3_filters'      # Directory of new dataset, without \\ at the end
NB_FILTERS = 3                                             # Number of filters
...
def calculate_crops_ccms(cfa_image, filter_channels, subsamples_crops, quant):
    ...
    for subsample_crop in subsamples_crops:
        ...
        cfa_ccms.append(get_3_filter_cfa_ccms(cfa_crop_quant, n_distance=CCM_DISTANCES, levels=quant))
        #print("--- CFA time: %s seconds ---" % (time.time() - start_time))
        #start_time = time.time()
        true_ccms.append(get_3_filter_true_ccms(filters_crop_quant, n_distance=CCM_DISTANCES, levels=quant))
...
def quantize_crops(sample, cfa_image, filter_channels, subsamples_crops):
    for quant in QUANTIZATION:
        ...
        save_obj(true_ccms, NEW_DATASET_DIR + sample + "\\true_ccms_{}.pkl".format(quant))
...
if __name__ == "__main__":
    ...
    # THIRD STEP
    # generate CFA images
    #generate_cfa_images(NEW_DATASET_DIR, get_cfa_image, FILTER_CONDITIONS_3)
