...
def greycomatrix_channels(image1, image2, distances, angles, channels, levels=None, symmetric=False,
                 normed=False):
    check_nD(image1, 2)
    check_nD(image2, 2)
    check_nD(distances, 1, 'distances')
    ...
    # count co-occurences
    P = glcm_loop_channels(image1, image2, distances, angles, levels, channels)
...
def glcm_loop_channels(image1, image2, distances, angles, levels, channels):
    ...
    for a_idx in range(angles.shape[0]):
        ...
        for d_idx in range(distances.shape[0]):
            ...
            offset_row = round(np.sin(angle*np.pi/180) * distance)
            offset_col = round(np.cos(angle*np.pi/180) * distance)
            start_row = max(0, -offset_row)
            end_row = min(rows, rows - offset_row)
            start_col = max(0, -offset_col)
            end_col = min(cols, cols - offset_col)
            # depending if it's rr, bb, gg
            if channels == 'r':
                # row is even, column is odd
                row_range = [x for x in list(range(start_row, end_row)) if x % 2 == 0]
                col_range = [x for x in list(range(start_col, end_col)) if x % 2 != 0]

                P = update_ccm(image1, image2, levels, P, a_idx, d_idx, offset_row, offset_col, row_range, col_range)

            elif channels == 'g':
                # row and column are even, row and column are odd
                row_range1 = [x for x in list(range(start_row, end_row)) if x % 2 == 0]
                col_range1 = [x for x in list(range(start_col, end_col)) if x % 2 == 0]
                row_range2 = [x for x in list(range(start_row, end_row)) if x % 2 != 0]
                col_range2 = [x for x in list(range(start_col, end_col)) if x % 2 != 0]

                P = update_ccm(image1, image2, levels, P, a_idx, d_idx, offset_row, offset_col, row_range1, col_range1)
                P = update_ccm(image1, image2, levels, P, a_idx, d_idx, offset_row, offset_col, row_range2, col_range2)
                """for r in row_range1:
                    for c in col_range1:
                        i = image1[r, c]
